...
function createWindow () {
  ...
  win.webContents.on('did-finish-load', () => {
    console.log('Renderer finished loading, sending onboarding status:', isOnboarded)
  ...
  // Also handle requests for onboarding status
  ipcMain.handle('get-onboarding-status', () => {
    console.log('Onboarding status requested:', isOnboarded)
    return { isOnboarded }
  })
...
// Complete local-only setup (skip cloud registration)
ipcMain.handle('complete-local-setup', async () => {
  try {
    ...
    throw new Error(`Failed to complete local setup: ${error.message}`)
  }
})
...
ipcMain.handle('start-tunnel', async () => {
  try {
    return new Promise((resolve, reject) => {
      // Resolve cloudflared.exe differently in dev vs packaged
      let cloudflaredPath
      if (app.isPackaged) {
        cloudflaredPath = path.join(process.resourcesPath, 'cloudflared.exe')
      } else {
        cloudflaredPath = path.join(__dirname, '../../cloudflared.exe')
      }
      // Fallbacks
      if (!fs.existsSync(cloudflaredPath)) {
        const alt1 = path.join(process.resourcesPath || '', 'cloudflared.exe')
        const alt2 = path.resolve(__dirname, '../../../cloudflared.exe')
        if (alt1 && fs.existsSync(alt1)) cloudflaredPath = alt1
        else if (fs.existsSync(alt2)) cloudflaredPath = alt2
      }
...
ipcMain.handle('get-tunnel-url', async () => {
  return tunnelUrl || store.get('cloudflare.tunnelUrl', null)
})

// Set a custom API base URL (advanced option)
ipcMain.handle('set-custom-api', async (event, { base }) => {
  const trainer = store.get('trainer') || {}
  store.set('trainer', { ...trainer, customApiBase: base || null })
  return { success: true }
})
    ...
    if (fs.existsSync(cloudflaredPath)) {
      ...
      tunnelProcess.stdout.on('data', (data) => {
        ...
        if (match && !tunnelUrl) {
          ...
          // Send tunnel URL to renderer window when ready
          setTimeout(() => {
            const windows = BrowserWindow.getAllWindows()
            if (windows.length > 0) {
      ...
      console.warn('⚠️  cloudflared.exe not found - tunnel not started')
    }
  } catch (error) {
