...
export const TrainerSignup = ({ onComplete }) => {
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      if (useLocalMode) {
        // Skip cloud registration, just use local mode
        setDeploymentStatus('Setting up local mode...');
        
        // Mark as onboarded without cloud setup
        await ipcRenderer.invoke('complete-local-setup');
        
        setDeploymentStatus('✓ Setup complete! Running in local mode.');
        ...
        <div className="signup-step">
          ...
          <div className="info-box" style={{ marginTop: 20 }}>
            ...
            <ul>
              ...
              {useLocalMode && (
                <li>✓ Local-only mode (no cloud features)</li>
              )}
            </ul>
          </div>
          ...
          <div className="form-group" style={{ marginTop: 20 }}>
            <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
              <input
                ...
                style={{ marginRight: 8 }}
              />
              Local Mode Only (skip cloud setup)
            </label>
            {useLocalMode && (
              <p style={{ fontSize: 12, color: '#666', marginTop: 4 }}>
                You can manage clients locally. Sharing features will be disabled.
              </p>
            )}
          </div>
